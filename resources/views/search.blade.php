<x-app-layout>
    <div class="bg-slate-50 py-10">
        <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            <div>
                <h1 class="text-xl font-semibold text-slate-900">Creer une tache</h1>
                <p class="mt-2 text-sm text-slate-500">Ajoutez une nouvelle tache a votre liste</p>
            </div>

            <form class="mt-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm" method="POST" action="{{ route('tasks.store') }}">
                @csrf
                <div class="space-y-5">
                    <div>
                        <label class="text-sm font-semibold text-slate-800">Titre *</label>
                        <input type="text" name="title" value="{{ old('title') }}" class="mt-2 w-full rounded-lg border border-slate-200 px-4 py-3 text-sm text-slate-700" placeholder="Titre de la tache">
                        @error('title')
                    ...
                    <div>
                        ...
                        <textarea rows="5" name="description" class="mt-2 w-full rounded-lg border border-slate-200 px-4 py-3 text-sm text-slate-700" placeholder="Description de la tache (optionnel)">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-2 text-xs text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="date" name="deadline" value="{{ old('deadline') }}" class="mt-2 w-full rounded-lg border border-slate-200 px-4 py-3 text-sm text-slate-700">
                        @error('deadline')
                            <p class="mt-2 text-xs text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" class="mt-2 w-full rounded-lg border border-slate-200 px-4 py-3 text-sm text-slate-700">
                            <option value="todo" @selected(old('status', 'todo') === 'todo')>A faire</option>
                            <option value="in_progress" @selected(old('status') === 'in_progress')>En cours</option>
                            <option value="done" @selected(old('status') === 'done')>Terminee</option>
                        </select>
                        @error('status')
                            <p class="mt-2 text-xs text-rose-600">{{ $message }}</p>
                        @enderror
